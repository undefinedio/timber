...
class Timber
{
	 ...
	 * @deprecated since 0.20.0
	 */
	static function get_posts_from_wp_query($query = [], $PostClass = 'TimberPost')
	{
		return TimberPostGetter::query_posts($query, $PostClass);
	}
	 ...
	 * @deprecated since 0.20.0
	 */
	static function get_posts_from_array_of_ids($query = [], $PostClass = 'TimberPost')
	{
	 ...
	 * @return mixed
	 */
	public static function get_terms($args = null, $maybe_args = [], $TermClass = 'TimberTerm')
	{
	...
	public static function get_context()
	{
		...
		if (function_exists('wp_nav_menu')) {
			...
			if (count($locations)) {
				$data['wp_nav_menu'] = wp_nav_menu(['container_class' => 'menu-header', 'echo' => false, 'menu_class' => 'nav-menu']);
			}
		}
	 ...
	 * @return bool|string
	 */
	public static function compile($filenames, $data = [], $expires = false, $cache_mode = TimberLoader::CACHE_USE_DEFAULT, $via_render = false)
	{
	 ...
	 * @return bool|string
	 */
	public static function render($filenames, $data = [], $expires = false, $cache_mode = TimberLoader::CACHE_USE_DEFAULT)
	{
		if ($expires === true) {
	 ...
	 * @return bool|string
	 */
	public static function get_sidebar($sidebar = '', $data = [])
	{
		if ($sidebar == '') {
			$sidebar = 'sidebar.php';
		}
	 ...
	 * @deprecated since 0.20.0
	 */
	public static function add_route($route, $callback, $args = [])
	{
	...
	{
		add_action('posts_request', [$this, 'cancel_query_posts_request']);
	}
	...
	public static function get_pagination($prefs = [])
	{
		...
		if ($next) {
			$data['next'] = ['link' => untrailingslashit($next), 'class' => 'page-numbers next'];
		}
		$prev = previous_posts(false);
		if ($prev) {
			$data['prev'] = ['link' => untrailingslashit($prev), 'class' => 'page-numbers prev'];
		}
		if ($paged < 2) {
			$data['prev'] = '';
		}
