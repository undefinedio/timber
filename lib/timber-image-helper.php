 ...
 * - most of the work is common to all filters (URL analysis, directory gymnastics, file caching, error management) and
 * done by private static functions
...
class TimberImageHelper
{
     ...
     *
     * @param string $src a url or path to the image (http://example.org/wp-content/uploads/2014/image.jpg) or
...
//-- end of public methods --//
    ...
    {
        $url = str_replace(ICL_LANGUAGE_CODE . '/', '', home_url());
        $is_external = TimberURLHelper::is_absolute($path) && !strstr($path, $url);
        if ($is_external) {
            $is_external = TimberURLHelper::is_absolute($path) && !strstr($path, $url);
        }
     ...
     * So if passed a value like my-pic.jpg, this function will delete my-pic-500x200-c-left.jpg,
     ...
     *
     * @param  int $base one of self::BASE_UPLOADS, self::BASE_CONTENT to indicate if file is an upload or a content
     *     (theme or plugin)
